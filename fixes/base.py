...
FIX: company_normalized was using .replace(" corp", "") which matched
" corp" INSIDE words like "corporation" → "microsoftoration".
Now uses regex with end-of-string anchors so it only strips suffixes.
"""
import hashlib
import re
...
class ScrapedJob(BaseModel):
    ...
    def company_normalized(self) -> str:
        ...
        FIX: Now uses regex with $ anchor to only strip suffixes at the END
        of the company name. Previously, .replace(" corp", "") would match
        inside "Microsoft Corporation" → "Microsoftoration".
        """
        name = self.company.lower().strip()
        
        # Remove common suffixes — ORDER MATTERS (longest first)
        # Using regex with end-of-string anchor ($) to avoid mid-word matches
        suffix_patterns = [
            r',?\s*incorporated$',
            r',?\s*corporation$',
            r',?\s*company$',
